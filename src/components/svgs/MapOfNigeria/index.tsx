...
export const MapOfNigeria: FC<IMapProps> = ({ statesData }) => {
  ...
  const handleMouseOver = (e: React.MouseEvent<SVGPathElement, MouseEvent>) => {
    ...
    // //   <p class="nameOfState">${state}</p>
    // // `;
    // // ref.current.style.left = x + 150 + "px";
    // // ref.current.style.top = y + 150 + "px";
  ...
  const handleClick = (e: React.MouseEvent<SVGPathElement, MouseEvent>) => {
    const state = e.target["dataset"]["tip"];
    console.log(e.target["dataset"]["tip"]);
    if (!state) return;

    history.push(`/d/organizations/${state.toLowerCase()}`);
  };
    ...
    <SVGWrapper>
      ...
      <ReactTooltip
        ...
        getContent={dataTip => {
          ...
          // console.log({ dataTip, state });

          return (
            <div>
              <p>
                <b>State:</b> {state ? state.name : "--"}
              </p>
              ...
              <p>
                ...
                {state ? state.numOfOrganizations : 0}
              </p>
            </div>
          );
        }}
